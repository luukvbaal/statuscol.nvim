...
local function sign_assign_segment(s, win)
  ...
  while segment <= signsegmentcount do
    ...
    if ss.lnum and not ss.wins[win].sclnu then
      goto next
    end
    if s.sign_name then -- legacy sign
      for j = 1, ss.notnamecount do
        if s.sign_name:find(ss.notname[j]) then
          goto next
        end
      end
      for j = 1, ss.namecount do
        if s.sign_name:find(ss.name[j]) then
          goto found
        end
      end
    else -- extmark sign
      for j = 1, ss.nottextcount do
        if s.wtext:find(ss.nottext[j]) then
          goto next
        end
      end
      for j = 1, ss.notnamespacecount do
        if s.ns:find(ss.notnamespace[j]) then
          goto next
        end
      end
      for j = 1, ss.textcount do
        if s.wtext:find(ss.text[j]) then
          goto found
        end
      end
      for j = 1, ss.namespacecount do
        if s.ns:find(ss.namespace[j]) then
          goto found
        end
      end
    end
...
local function sign_cache_add(win, s, name)
  if not s.sign_name then
    s.ns = nsmap[s.ns_id]
  end
  s.wtext = s.sign_text:gsub('%s', '')
  s.segment = sign_assign_segment(s, win)
  if s.segment and signsegments[s.segment].colwidth == 1 then
    s.sign_text = s.wtext
  end
  sign_cache[name] = s
end
...
local function get_click_args(minwid, clicks, button, mods)
  ...
  args.text = text ~= ' ' and text
    or f.screenstring(args.mousepos.screenrow, args.mousepos.screencol - 1)
...
local function get_fold_action(minwid, clicks, button, mods)
  ...
  local type = args.text == fold.open and 'FoldOpen'
    or args.text == fold.close and 'FoldClose'
    or 'FoldOther'
...
local function get_sign_action_inner(args)
  ...
  for _, s in
    ipairs(
      a.nvim_buf_get_extmarks(0, -1, { row, 0 }, { row, -1 }, { type = 'sign', details = true })
    )
  do
...
local function get_lnum_action(minwid, clicks, button, mods)
  ...
  if lnumfunc and cargs.sclnu and get_sign_action_inner(args) then
    return
  end
...
local function place_signs(win, signs)
  ...
  for i = 1, #signs do
    local s = signs[i][4]
    if not s.sign_text then
      goto nextsign
    end

    local name = s.sign_name or s.sign_text
    if not s.sign_hl_group then
      s.sign_hl_group = 'NoTexthl'
    end
    if not s.sign_name then
      name = name .. s.sign_hl_group
    end
    if not sign_cache[name] then
      sign_cache_add(win, s, name)
    end
    local sign = sign_cache[name]
    if not sign.segment then
      goto nextsign
    end
    ...
    if ss.foldclosed then
      if not lines[lnum] then
        lines[lnum] = f.foldclosed(lnum)
      end
      if lines[lnum] > 0 then
        lnum = lines[lnum]
        for j = lnum + 1, f.foldclosedend(lnum) do
          lines[j] = lnum
        end
      end
    end

    if not sss[lnum] then
      sss[lnum] = {}
    end
...
M.get_statuscol_string = function()
  ...
  for i = 1, segmentcount do
    ...
    formatargret[i] = (s.cond == true or s.cond(args))
        and (s.textfunc and s.text(args, s) or s.text)
      or ''
  end
...
function M.setup(user)
  ...
  local cfgsegments = cfg.segments
    or {
  ...
  for i = 1, #cfgsegments do
    ...
    if ss then
      ...
      if ss.fillcharhl then
        ss.fillcharhl = '%#' .. ss.fillcharhl .. '#'
      end
      if setscl ~= false then
        setscl = true
      end
      if not segment.text then
        segment.text = { builtin.signfunc }
      end
    end
    if segment.hl then
      formatstr = formatstr .. '%%#' .. segment.hl .. '#'
    end
    if segment.click then
      formatstr = formatstr .. '%%@' .. segment.click .. '@'
    end
    ...
    if segment.click then
      formatstr = formatstr .. '%%T'
    end
    if segment.hl then
      formatstr = formatstr .. '%%*'
    end
  end
  if setscl and o.scl ~= 'number' then
    o.scl = 'no'
  end
  ...
  if cfg.setopt then
    ...
    for _, tab in ipairs(a.nvim_list_tabpages()) do
      for _, win in ipairs(a.nvim_tabpage_list_wins(tab)) do
        ...
        if
          not a.nvim_buf_get_name(buf):find('vim._extui')
          and not contains(cfg.ft_ignore or {}, a.nvim_get_option_value('ft', { buf = buf }))
          and not contains(cfg.bt_ignore or {}, a.nvim_get_option_value('bt', { buf = buf }))
        then
  ...
  if cfg.ft_ignore then
    a.nvim_create_autocmd(
      'FileType',
      { group = id, pattern = cfg.ft_ignore, command = 'setlocal stc=' }
    )
